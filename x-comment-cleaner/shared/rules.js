/**
 * X Spam Reply Cleaner - Unified Core Rules & Engine (v2.0)
 * 
 * Standalone, zero-dependency, self-contained module for:
 * 1. 8 Categorized Keyword Packs with max_gap Phrase Matching
 * 2. 64-bit SimHash Fuzzy Variant & Campaign Fingerprinting
 * 3. Account & External Link Domain Heuristics
 * 4. Pure Number & Bot Mention Pattern Detectors
 * 5. Whitelist & OP Protection Filters
 */
...
// === 3. Heuristics & Text Normalization ===
...
function evaluateTextAgainstPacks(text, {
  ...
  // 1. Check preset keyword packs
  for (const pack of KEYWORD_PACKS) {
    if (packSettings && packSettings[pack.id] === false) continue;
...
function evaluateReplySpam({
  ...
  if (text) {
    ...
    if (cfg.filterDuplicates && duplicateTracker) {
      const compText = normalizeForSimhash(text);
