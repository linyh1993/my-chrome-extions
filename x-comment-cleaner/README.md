...
4. **X 原生拉黑与防封流控机制 (Native Block & Anti-Ban Rate Limiter)**：
   - **真实请求官方接口**：调用 X 官方原生端点（优先解析 UserByScreenName 获取 `rest_id` 调用 `1.1/blocks/create.json`，支持 `screen_name` 降级兜底），全端手机/Web同步生效。
   - **防封流控保护与安全间隔**：内置先进的 FIFO 排队机制，单次请求间隔保持在 2.5s~4.5s 随机浮动（模拟真人操作节奏，杜绝并发冲击）；若触发官方 HTTP 429 自动进入 60 秒冷却退避，彻底避免账号被 X 风控或封禁。
   - **多标签页防重同步**：拉黑记录自动存入浏览器本地缓存，多标签页协同避免重复发起请求。
   - **灵活开关与配置**：默认开启自动拉黑，可随时在 Popup 面板开启/关闭，所有配置实时保存在浏览器存储（`chrome.storage.sync`）内。
   - 折叠条中直接提供 **「顺手拉黑 / 撤销拉黑」** 与 **「一键加白」**。
