...
### 3. 👥 Follow 批量转列表 (`x-follow-to-list`)
- **无感自动捕获**：在关注/粉丝页滚动浏览时，底座自动拦截 GraphQL 并结构化解析账号（支持至多 5000 条缓存）；
- **多维组合筛选**：支持关键词搜索（昵称/Handle/Bio）、仅互关 (Mutual)、仅认证蓝标、粉丝数区间等快速过滤；
- **本地数据导出**：支持一键将所选或筛选后的关注者列表导出为带 UTF-8 BOM 的 CSV 格式；
- **防风控安全导入**：批量加入指定 List，内置 2.5s~4.5s 随机安全间隔，遇 X 官方 429 限流自动触发 45 秒冷却退避保护账号。

### 4. 🔥 推文流速与爆帖监控 (`x-viral-monitor`)
- **实时流速分析**：每小时浏览量流速 (views/h) 实时计算，三级热度标识（🌱 正常 / 🚀 热门 / 🔥 爆帖）；
- **爆帖综合指数**：多维加权评分 (0~100 分：流速 40% + 互动率 25% + 转赞比 20% + 藏赞比 15%)，悬浮展开指标大盘卡；
- **书签收藏数还原**：补齐 X 官方默认隐藏的时间线推文书签计数；
- **一键结构化 Markdown**：推文卡片注入快捷按钮，一键复制带图文引用格式的 Markdown；
- **大盘流速排行榜**：Sidepanel 监控大盘实时汇总当前高热推文，支持一键定位高亮页面推文。

### 5. 🚀 规划与接入中能力
- **高级搜索助手**（借鉴 `advanced-search-for-x-twitter`）：可视化搜索语法生成器与预设管理。
