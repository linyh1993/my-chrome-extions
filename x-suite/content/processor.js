...
const XcfProcessor = (() => {
  ...
  function entryMatchesArticle(entry, article, meta) {
    const tid = S().adapter?.getTweetId?.(article);
    const entryTid = String(entry.tweetId || '').trim();
    if (entryTid && tid && entryTid === tid) return true;

    const eh = XcfSettings.normalizeHandle(entry.handle);
    const ah = XcfSettings.normalizeHandle(meta?.handle);
    const norm = (t) =>
      String(t || '')
        .replace(/\s+/g, ' ')
        .trim()
        .slice(0, 120);
    const et = norm(entry.text);
    const at = norm(meta?.text);
    if (eh && ah && eh === ah && et && at && et === at) return true;

    const key = XcfArchiveLog.captureKey(article, meta);
    if (entry.id && key && entry.id === key) return true;
    return false;
  }

  async function unclassifyEntry(entry) {
    if (!entry) return;
    await XcfRuntime.withPausedObserver(() => {
      for (const article of S().adapter.findArticles()) {
        const meta = S().adapter.extractMeta(article);
        if (!entryMatchesArticle(entry, article, meta)) continue;
        const key = XcfArchiveLog.captureKey(article, meta);
        if (key) S().loggedArchiveKeys.delete(key);
